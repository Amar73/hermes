...
## 6. Авторизация Claude Code (Pro, OAuth) — ⏸ ОТЛОЖЕНО (2026-07-09)

**Решение пользователя: пока не логинить Pro-подписки (Claude Code / Gemini CLI / Codex) на headless-сервере вообще.** Причина: риск бана аккаунта — Anthropic (и вероятно другие провайдеры) может расценить программную/автономную эксплуатацию персональной Pro-подписки через агентный пайплайн на удалённом сервере как нарушение условий использования. Это отменяет не только этот шаг, но и всю схему раздела 10 (Writer=Claude Code Pro, Reviewer=Gemini CLI Pro, Auditor=Codex Pro) в её первоначальном виде — **сейчас в работе остаётся только доступ через OpenRouter** (см. шаг 5, уже настроено и проверено). Не запускать `claude auth login` / `gemini auth login` / `codex login` под `paperclip` без явного пересмотра этого решения пользователем.

Правильная команда (если/когда решение изменится) — не `claude login`, а `claude auth login` (проверено 2026-07-09: `claude login` не существует как подкоманда в установленной версии, только `claude auth {login,logout,status}`). Требует интерактивного терминала (TUI буферизует stdout, если это не pty) — рабочий способ гонять такое на headless-сервере: `tmux new-session -d -s claude-login 'claude auth login'`, затем `tmux capture-pane -p` чтобы прочитать URL, и `tmux send-keys`/повторный `capture-pane` для остального диалога.

Оригинальный текст шага (для истории, если решение будет пересмотрено):
```bash
sudo -u paperclip claude auth login
```
Откроется ссылка для OAuth — переходите по ней в браузере на своём компьютере, входите аккаунтом с подпиской Pro, вставляете код обратно в терминал сервера.

---

## 7. Авторизация Gemini CLI (Google AI Pro, OAuth) — ⏸ ОТЛОЖЕНО (см. шаг 6)

Тот же мораторий, что и в шаге 6, действует и здесь — не выполнять без пересмотра решения.

Оригинальный текст шага (для истории):
```bash
...
**⏸ Обновление 2026-07-09: эта таблица приостановлена в исходном виде.** Пользователь решил не логинить Pro-подписки (Claude Code/Gemini CLI/Codex) на headless-сервере из-за риска бана аккаунта за программную/автономную эксплуатацию персональной подписки (см. шаг 6). Пока в работе остаётся только доступ через OpenRouter (API, оплата по использованию — не персональная подписка, риск другой природы). Roles Writer/Reviewer/Auditor нужно будет либо: (a) пересобрать на моделях через OpenRouter вместо Pro CLI-логинов, либо (b) оставить как есть и вернуться к вопросу отдельно — решение пока не принято, не продолжать шаги 6/7/10.1 без явного ОК пользователя.
...
### 10.1 Авторизация Codex CLI (Pro, OAuth) — ⏸ ОТЛОЖЕНО (см. шаг 6)

Тот же мораторий на логин Pro-подписок на headless-сервере, что и в шаге 6 — не выполнять без пересмотра решения. Оригинальный текст (для истории):
```bash
...
3. Перезапустите гейтвей: `sudo systemctl restart hermes-gateway`

**(Выполнено 2026-07-09, но с оговоркой)**: ключ записан и гейтвей перезапущен, однако **Hermes Agent не читает `PERPLEXITY_API_KEY` как провайдера** — единственное упоминание в его коде это защитный blocklist (переменные, которые не должны утекать в песочницу терминала), в `hermes status` ключ не отображается. В Paperclip нативного провайдера Perplexity тоже нет — единственная находка это опциональный сторонний скилл `last30days` (github: mvanhorn/last30days-skill, не установлен по умолчанию), который использует Perplexity как один из многих источников (наравне с Reddit/X/YouTube/HN), а не как основной бэкенд роли Researcher. Реальная схема для роли Researcher станет ясна только при настройке org chart через дашборд (шаг 10.3) — не изобретать интеграцию заранее.
...
- [ ] ⏸ `claude auth login` — ОТЛОЖЕНО 2026-07-09, риск бана Pro-аккаунта за автоматизацию на headless-сервере, не выполнять без пересмотра
- [ ] ⏸ `gemini auth login` — ОТЛОЖЕНО 2026-07-09, та же причина
