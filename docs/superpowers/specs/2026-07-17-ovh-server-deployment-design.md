...
11. **Обновление Hermes из дашборда роняет сам дашборд** (обнаружено сразу после развёртывания): апдейтер шлёт SIGTERM процессу дашборда, а `Restart=on-failure` в systemd не перезапускает юнит после TERM (systemd считает это чистой остановкой) — в браузере остаётся вечный спиннер «идёт обновление». Исправлено: в `hermes-dashboard.service` политика заменена на `Restart=always`. Gateway этой проблеме не подвержен (перезапускается сам). Autostash апдейтера (`git stash` в `~/.hermes/hermes-agent`) возвращается на место автоматически.
